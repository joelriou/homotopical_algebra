...
begin
  ...
  { haveI : lifting L₁ W₁ (I ⋙ L₂) I' := ⟨h.iso⟩,
    ...
    { calc (I ⋙ L₂) ⋙ R' ≅ I ⋙ L₂ ⋙ R' : functor.associator _ _ _
      ... ≅ I ⋙ R : iso_whisker_left _ (lifting.iso W₂)
      ... ≅ L₁ : ε, },
...
lemma lifting_equivalence_counit_iso_app (X : C₂) :
  ...
  I'.map_iso (hR.iso.app X) ≪≫ η.app X :=
begin
  ext,
  dsimp [lifting_equivalence, equivalence.mk],
  simp only [lifting.uniq_hom_app, lifting.of_isos_iso, iso.refl_symm, lifting.comp_right_iso,
    iso.trans_hom, iso_whisker_left_hom, iso.refl_hom, whisker_left_id', iso_whisker_right_hom,
    id_comp, nat_trans.comp_app, whisker_right_app, lifting.id_iso, functor.right_unitor_inv_app,
    comp_id],
end
